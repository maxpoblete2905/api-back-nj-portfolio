...
export class AcademicService {
  ...
  private formatAcademic(doc: any): Academic {
    try {
      ...
      this.logger.error(
        `Error formatting academic record: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async create(
    createDto: CreateAcademicDto,
  ): Promise<StandardResponse<Academic>> {
    ...
    try {
      ...
      if (!createResult.success || !createResult.data) {
        const errorMessage =
          createResult.message || 'Failed to create academic record';
      ...
      this.logger.log(
        `Academic record created successfully with ID: ${doc.id}`,
      );
      return {
        success: true,
  ...
  async findAll(): Promise<StandardResponse<Academic[]>> {
    ...
    try {
      const getResult = await this.firestoreService.getDocs<Academic>(
        this.collectionName,
      );

      if (!getResult.success || !getResult.data) {
        const errorMessage =
          getResult.message || 'Failed to fetch academic records';
      ...
      const academics = getResult.data.docs.map((doc) =>
        this.formatAcademic(doc),
      );
      ...
      this.logger.error(
        `Failed to fetch academic records: ${error.message}`,
        error.stack,
      );
      return {
        success: false,
  ...
  async findOne(id: string): Promise<StandardResponse<Academic>> {
    ...
    try {
      const getResult = await this.firestoreService.getDoc<Academic>(
        this.collectionName,
        id,
      );
      ...
      this.logger.error(
        `Failed to fetch academic record ${id}: ${error.message}`,
        error.stack,
      );
      return {
        success: false,
  ...
  async update(
    ...
    try {
      ...
      if (!updateResult.success) {
        this.logger.error(
          `Failed to update academic record ${id}: ${updateResult.message}`,
        );
        return {
          success: false,
      ...
      const updatedDoc = await this.firestoreService.getDoc(
        this.collectionName,
        id,
      );
      ...
      this.logger.error(
        `Failed to update academic record ${id}: ${error.message}`,
        error.stack,
      );
      return {
        success: false,
  ...
  async remove(id: string): Promise<StandardResponse<void>> {
    ...
    try {
      ...
      if (!deleteResult.success) {
        this.logger.error(
          `Failed to delete academic record ${id}: ${deleteResult.message}`,
        );
        return {
          success: false,
      ...
      this.logger.error(
        `Failed to delete academic record ${id}: ${error.message}`,
        error.stack,
      );
      return {
        success: false,
  ...
  async seedInitialData(): Promise<StandardResponse<void>> {
    ...
    try {
      ...
      this.logger.error(
        `Failed to seed academic data: ${error.message}`,
        error.stack,
      );
      return {
        success: false,
