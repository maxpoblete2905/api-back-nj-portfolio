...
export class ContactsService {
    ...
    private formatContact(doc: any): Contact {
        try {
            ...
            this.logger.error(`Error formatting contact: ${error.message}`, error.stack);
            throw error;
        }
    }

    async create(createDto: CreateContactDto): Promise<StandardResponse<Contact>> {
        ...
        try {
            ...
            const createResult = await this.firestoreService.createDoc<typeof contactData>(
                this.collectionName,
                contactData
            );
            ...
            this.cacheService.del('all_contacts');
    ...
    async findAll(): Promise<StandardResponse<Contact[]>> {
        ...
        try {
            const getResult = await this.firestoreService.getDocs<Contact>(this.collectionName);
            ...
            const contacts = getResult.data.docs.map(doc => this.formatContact(doc));
    ...
    async findOne(id: string): Promise<StandardResponse<Contact>> {
        ...
        try {
            const getResult = await this.firestoreService.getDoc<Contact>(this.collectionName, id);
            ...
            this.logger.error(`Failed to fetch contact ${id}: ${error.message}`, error.stack);
            return {
                success: false,
    ...
    async update(id: string, updateDto: UpdateContactDto): Promise<StandardResponse<Contact>> {
        ...
        try {
            ...
            const updateResult = await this.firestoreService.updateDoc<UpdateContactDto>(
                this.collectionName,
                id,
                updateDto
            );
            ...
            if (!updateResult.success) {
                this.logger.error(`Failed to update contact ${id}: ${updateResult.message}`);
                return {
                    success: false,
            ...
            this.cacheService.del(`contact_${id}`);
            this.cacheService.del('all_contacts');
            ...
            this.logger.error(`Failed to update contact ${id}: ${error.message}`, error.stack);
            return {
                success: false,
    ...
    async remove(id: string): Promise<StandardResponse<void>> {
        ...
        try {
            ...
            if (!deleteResult.success) {
                this.logger.error(`Failed to delete contact ${id}: ${deleteResult.message}`);
                return {
                    success: false,
            ...
            this.logger.error(`Failed to delete contact ${id}: ${error.message}`, error.stack);
            return {
                success: false,
    ...
    async findByStatus(status: string): Promise<StandardResponse<Contact[]>> {
        ...
        try {
            ...
            if (!queryResult.success || !queryResult.data) {
                const errorMessage = queryResult.message || 'Failed to query contacts by status';
            ...
            const contacts = queryResult.data.docs.map(doc => this.formatContact(doc));
            this.logger.debug(`Found ${contacts.length} contacts with status ${status}`);
            ...
            this.logger.error(`Failed to find contacts by status: ${error.message}`, error.stack);
            return {
                success: false,
