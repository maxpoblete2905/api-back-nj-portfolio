...
export class ProjectsService {
  ...
  private formatProject(doc: any): Project {
    try {
      ...
      this.logger.error(
        `Error formatting project: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async create(
    createProjectDto: Omit<Project, 'id'>,
    ...
    try {
      const createResult = await this.firestoreService.createDoc<
        Omit<Project, 'id'>
      >(this.collectionName, createProjectDto);
  ...
  async findAll(): Promise<StandardResponse<Project[]>> {
    ...
    try {
      const getResult = await this.firestoreService.getDocs<Project>(
        this.collectionName,
      );
      ...
      const projects = getResult.data.docs.map((doc) =>
        this.formatProject(doc),
      );
  ...
  async findOne(id: string): Promise<StandardResponse<Project>> {
    ...
    try {
      const getResult = await this.firestoreService.getDoc<Project>(
        this.collectionName,
        id,
      );
  ...
  async update(
    ...
    try {
      ...
      const updateResult = await this.firestoreService.updateDoc<
        Partial<Project>
      >(this.collectionName, id, updateProjectDto);

      if (!updateResult.success) {
        this.logger.error(
          `Failed to update project ${id}: ${updateResult.message}`,
        );
        return {
          success: false,
  ...
  async remove(id: string): Promise<StandardResponse<void>> {
    ...
    try {
      ...
      if (!deleteResult.success) {
        this.logger.error(
          `Failed to delete project ${id}: ${deleteResult.message}`,
        );
        return {
          success: false,
