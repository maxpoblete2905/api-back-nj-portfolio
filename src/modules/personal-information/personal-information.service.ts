...
export class PersonalInfoService {
  ...
  private formatPersonalInfo(doc: any): PersonalInformation {
    try {
      ...
      this.logger.error(
        `Error formatting personal info: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async create(
    createDto: CreatePersonalInfoDto,
  ): Promise<StandardResponse<PersonalInformation>> {
    this.logger.log('Creating new personal information');
    try {
      const createResult =
        await this.firestoreService.createDoc<CreatePersonalInfoDto>(
          this.collectionName,
          createDto,
        );
      ...
      if (!createResult.success || !createResult.data) {
        const errorMessage =
          createResult.message || 'Failed to create personal info';
      ...
      this.logger.error(
        `Failed to create personal info: ${error.message}`,
        error.stack,
      );
      return {
        success: false,
  ...
  async findAll(): Promise<StandardResponse<PersonalInformation[]>> {
    ...
    try {
      const getResult =
        await this.firestoreService.getDocs<PersonalInformation>(
          this.collectionName,
        );

      if (!getResult.success || !getResult.data) {
        const errorMessage =
          getResult.message || 'Failed to fetch personal info';
      ...
      const personalInfoList = getResult.data.docs.map((doc) =>
        this.formatPersonalInfo(doc),
      );
      this.logger.debug(
        `Found ${personalInfoList.length} personal info records`,
      );
      ...
      this.logger.error(
        `Failed to fetch personal info: ${error.message}`,
        error.stack,
      );
      return {
        success: false,
  ...
  async findOne(id: string): Promise<StandardResponse<PersonalInformation>> {
    ...
    try {
      const getResult = await this.firestoreService.getDoc<PersonalInformation>(
        this.collectionName,
        id,
      );
      ...
      this.logger.error(
        `Failed to fetch personal info ${id}: ${error.message}`,
        error.stack,
      );
      return {
        success: false,
  ...
  async update(
    ...
    try {
      ...
      const updateResult =
        await this.firestoreService.updateDoc<UpdatePersonalInfoDto>(
          this.collectionName,
          id,
          updateDto,
        );
      ...
      if (!updateResult.success) {
        this.logger.error(
          `Failed to update personal info ${id}: ${updateResult.message}`,
        );
        return {
          success: false,
      ...
      this.logger.error(
        `Failed to update personal info ${id}: ${error.message}`,
        error.stack,
      );
      return {
        success: false,
  ...
  async remove(id: string): Promise<StandardResponse<void>> {
    ...
    try {
      ...
      if (!deleteResult.success) {
        this.logger.error(
          `Failed to delete personal info ${id}: ${deleteResult.message}`,
        );
        return {
          success: false,
      ...
      this.logger.error(
        `Failed to delete personal info ${id}: ${error.message}`,
        error.stack,
      );
      return {
        success: false,
