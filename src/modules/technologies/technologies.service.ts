...
export class TechnologiesService {
    ...
    private formatTechnology(doc: any): Technology {
        try {
            ...
            this.logger.error(`Error formatting technology: ${error.message}`, error.stack);
            throw error;
        }
    }

    async create(createDto: CreateTechnologyDto): Promise<StandardResponse<Technology>> {
        this.logger.log('Creating new technology');
        try {
            const createResult = await this.firestoreService.createDoc<CreateTechnologyDto>(
                this.collectionName,
                createDto
            );

            if (!createResult.success || !createResult.data) {
                const errorMessage = createResult.message || 'Failed to create technology';
            ...
            this.logger.error(`Failed to create technology: ${error.message}`, error.stack);
            return {
                success: false,
    ...
    async findAll(): Promise<StandardResponse<Technology[]>> {
        ...
        try {
            const getResult = await this.firestoreService.getDocs<Technology>(this.collectionName);

            if (!getResult.success || !getResult.data) {
                const errorMessage = getResult.message || 'Failed to fetch technologies';
            ...
            const technologies = getResult.data.docs.map(doc => this.formatTechnology(doc));
            this.logger.debug(`Found ${technologies.length} technologies`);

            return {
                success: true,
                message: technologies.length > 0
            ...
            this.logger.error(`Failed to fetch technologies: ${error.message}`, error.stack);
            return {
                success: false,
    ...
    async findOne(id: string): Promise<StandardResponse<Technology>> {
        ...
        try {
            const getResult = await this.firestoreService.getDoc<Technology>(this.collectionName, id);
            ...
            this.logger.error(`Failed to fetch technology ${id}: ${error.message}`, error.stack);
            return {
                success: false,
    ...
    async update(id: string, updateDto: UpdateTechnologyDto): Promise<StandardResponse<Technology>> {
        ...
        try {
            ...
            const updateResult = await this.firestoreService.updateDoc<UpdateTechnologyDto>(
                this.collectionName,
                id,
                updateDto
            );
            ...
            if (!updateResult.success) {
                this.logger.error(`Failed to update technology ${id}: ${updateResult.message}`);
                return {
                    success: false,
            ...
            this.logger.error(`Failed to update technology ${id}: ${error.message}`, error.stack);
            return {
                success: false,
    ...
    async remove(id: string): Promise<StandardResponse<void>> {
        ...
        try {
            ...
            if (!deleteResult.success) {
                this.logger.error(`Failed to delete technology ${id}: ${deleteResult.message}`);
                return {
                    success: false,
            ...
            this.logger.error(`Failed to delete technology ${id}: ${error.message}`, error.stack);
            return {
                success: false,
